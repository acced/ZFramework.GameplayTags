...
namespace GameplayTags.Editor
{
    internal static class GameplayTagSettingsProvider
    {
        ...
        {
            var provider = new SettingsProvider("Project/ZFramework/Gameplay Tags", SettingsScope.Project)
            {
                label = "Gameplay Tags",
        ...
        private static void DrawSettings(string searchContext)
        {
            ...
            if (settings == null)
            {
                EditorGUILayout.HelpBox(
                    "No GameplayTagSettings asset exists at Resources/" + GameplayTagSettings.ResourcesPath + ".",
                    MessageType.Warning);
                if (GUILayout.Button("Create Gameplay Tag Settings", GUILayout.Width(240f)))
                    settings = GameplayTagEditorUtility.CreateSettings();
                return;
            }
            ...
            EditorGUILayout.LabelField("Content Hash", "0x" + settings.ComputeContentHash().ToString("X8"));

            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button("Open Tag Manager", GUILayout.Width(180f)))
                GameplayTagManagerWindow.Open();
            if (GUILayout.Button("Generate C# API", GUILayout.Width(180f)))
